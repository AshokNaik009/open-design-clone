...
function resolvePackageManagerInvocation() {
  ...
  return { argsPrefix: [], command: process.platform === "win32" ? "npm.cmd" : "npm" };
}
...
for (const target of buildTargets) {
  ...
  const result = spawnSync(
    ...
    [...packageManager.argsPrefix, "run", "build", "--workspace", target],
    {
      cwd: repoRoot,
      stdio: "inherit",
    },
...
if (needsRebuild) {
  ...
  const rebuild = spawnSync(
    packageManager.command,
    [...packageManager.argsPrefix, "rebuild", "better-sqlite3", "--workspace", "@open-design/daemon"],
  ...
  if (rebuild.status !== 0) {
    process.stderr.write(
      "postinstall: better-sqlite3 rebuild failed.\n" +
        "Install build tools (python3, make, g++ or clang++) then run: npm install\n",
    );
